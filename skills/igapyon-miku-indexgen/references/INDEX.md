---
title: igapyon-miku-indexgen references
topics:
  - miku-indexgen
  - references
  - navigation
  - index-json
  - agent-skill
---

# References

This is the curated reference guide for agents using `igapyon-miku-indexgen`.
For a generated file inventory of the installed skill package, read
[`../index.json`](../index.json).

Use this file when you need to choose which reference document to read next.
Use `../index.json` when you need a generated overview of files in the skill
package.

- [usage.md](usage.md): agent-facing usage guide, request examples, runtime examples, and version notes
- [workflow/indexgen-workflow.md](workflow/indexgen-workflow.md): generation workflow
- [workflow/agent-skill-index-workflow.md](workflow/agent-skill-index-workflow.md): applying generated index.json to Agent Skills packages
- [runtime/operations-map.md](runtime/operations-map.md): CLI operations and backend mapping
- [runtime/input-files-spec.md](runtime/input-files-spec.md): directory-wide input, supported input formats, encoding, metadata rules, and recommended Markdown authoring
- [runtime/miku-indexgen-frontmatter-spec.md](runtime/miku-indexgen-frontmatter-spec.md): supported YAML front matter metadata fields, date metadata, structured sources, and authoring strategy
- [runtime/index-json-spec.md](runtime/index-json-spec.md): generated index.json structure and maintenance rules
- [../examples/indexgen-examples.md](../examples/indexgen-examples.md): common command examples
- [../examples/maven-index-generation.md](../examples/maven-index-generation.md): Maven generation examples for Agent Skills indexes
- [../templates/README.md](../templates/README.md): template directory notes and reusable Maven templates
