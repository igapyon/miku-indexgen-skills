...
test("resolves bundled runtime artifacts", () => {
  ...
  assert.equal(javaArtifact.name, "miku-indexgen-1.2.1.jar");
  assert.equal(javaArtifact.version, "1.2.1");
  assert.equal(nodeArtifact.name, "miku-indexgen-1.2.0.mjs");
  assert.equal(nodeArtifact.version, "1.2.0");
