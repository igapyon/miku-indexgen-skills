...
test("resolves bundled runtime artifacts", () => {
  ...
  assert.equal(javaArtifact.name, "miku-indexgen-1.6.2.jar");
  assert.equal(javaArtifact.version, "1.6.2");
  assert.equal(nodeArtifact.name, "miku-indexgen-1.6.2.mjs");
  assert.equal(nodeArtifact.version, "1.6.2");
